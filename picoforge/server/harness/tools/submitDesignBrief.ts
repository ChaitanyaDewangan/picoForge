...
export const CATEGORIES = [
  "rotor_fan", "rotor_turbine", "bracket", "enclosure",
  "heat_exchanger", "gear", "shaft", "duct", "lattice_part", "generic",
] as const;
...
const zodInput = z.object({
  ...
  envelope_mm:    z.object({ x: z.number().positive(), y: z.number().positive(), z: z.number().positive() }),
...
const jsonSchema = {
  ...
  properties: {
    ...
    physics_checks: { type: "array", items: { type: "object", properties: {
      ...
      computed: { type: "number" }, limit: { type: "number" }, pass: { type: "boolean" },
    }, required: ["name","formula","computed","limit","pass"] } },
    envelope_mm: { type: "object", properties: { x: { type: "number" }, y: { type: "number" }, z: { type: "number" } }, required: ["x","y","z"] },
...
export function validateBrief(input: Input): BriefValidationResult {
  ...
  for (const ax of ["x","y","z"] as const) {
    if (env[ax] > proj[ax] * 1.05) {
      violations.push(`envelope_mm.${ax} (${env[ax]}) exceeds project max ${proj[ax]} mm × 1.05 = ${proj[ax] * 1.05}`);
    }
  }
  ...
  for (const p of input.parameters) {
    if (p.unit === "mm" && p.value > maxAxis * 1.05) {
      violations.push(`parameter "${p.name}" (${p.value} mm) exceeds max envelope axis ${maxAxis} mm × 1.05 = ${maxAxis * 1.05} mm`);
    }
  }
  ...
  if (cells > HARD_CELL_CAP) {
    ...
    warnings.push(`voxel budget ${cells.toLocaleString()} cells exceeds soft cap ${SOFT_CELL_CAP.toLocaleString()} — expect slow build`);
  }
  ...
  for (const c of input.physics_checks) {
    if (!c.pass) {
      ...
      if (!waived) {
        violations.push(`physics_check "${c.name}" has pass:false — fix the design or add an assumption explicitly waiving it`);
      } else {
  ...
  return { ok: violations.length === 0, violations, warnings, cellEstimate: cells, suggestedVoxelMm };
}
...
export const submitDesignBriefTool: ToolModule<Input> = {
  ...
  zodInput,
  jsonSchema,
  def: makeDef("submit_design_brief",
    "Register the engineering design brief. The system validates envelope, voxel budget, units, and category-required physics checks. You MUST submit an accepted brief before building. Resubmit (same tool) when the design direction changes materially.",
    jsonSchema,
  ),
  ...
  async execute(input, ctx: RunCtx) {
    try {
      ...
      if (!parsed.success) return err(new Error(`submit_design_brief validation: ${parsed.error.message}`));
      ...
      return ok({
        ...
        message: `Brief accepted: "${data.title}" (${data.category}, ${data.parameters.length} params, est. ${validation.cellEstimate.toLocaleString()} cells)`,
      });
    } catch (e) {
